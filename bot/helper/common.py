...
class TaskConfig:
    ...
    async def proceed_auto_remove(self, dl_path, gid):
        ...
        auto_remove = self.user_dict.get("AUTO_REMOVE", False)
        if not auto_remove:
            return dl_path
        ...
        kept_on = self.user_dict.get("AUTO_REMOVE_KEPT_ENABLE", False)
        kept_spec = self.user_dict.get("AUTO_REMOVE_KEPT", "")

        remove_on = self.user_dict.get("AUTO_REMOVE_REMOVE_ENABLE", False)
        remove_spec = self.user_dict.get("AUTO_REMOVE_REMOVE", "")

        reorder_on = self.user_dict.get("AUTO_REMOVE_REORDER_ENABLE", False)
        reorder_spec = self.user_dict.get("AUTO_REMOVE_REORDER", "")

        for f_path in all_files:
            if self.is_cancelled:
                return False
            if kept_on and kept_spec:
                async with task_dict_lock:
                    task_dict[self.mid] = FFmpegStatus(self, ffmpeg, gid, "Auto Remove Kept")
                await ffmpeg.auto_remove_kept(f_path, kept_spec)

            if remove_on and remove_spec:
                async with task_dict_lock:
                    task_dict[self.mid] = FFmpegStatus(self, ffmpeg, gid, "Auto Remove Remove")
                await ffmpeg.auto_remove_remove(f_path, remove_spec)

            if reorder_on and reorder_spec:
                async with task_dict_lock:
                    task_dict[self.mid] = FFmpegStatus(self, ffmpeg, gid, "Auto Remove Reorder")
                await ffmpeg.auto_remove_reorder(f_path, reorder_spec)
    ...
    async def proceed_audio_split(self, dl_path, gid):
        ...
        split_on = self.user_dict.get("AUDIO_SPLIT_ENABLE", False)
        split_spec = self.user_dict.get("AUDIO_SPLIT", "")
        if not split_on or not split_spec:
            return dl_path
        ...
        for f_path in all_files:
            if self.is_cancelled:
                return False
            async with task_dict_lock:
                task_dict[self.mid] = FFmpegStatus(self, ffmpeg, gid, "Audio Split")
            res = await ffmpeg.audio_split(f_path, split_spec)
            if res:
                split_created = True

        if split_created and self.is_file:
            self.is_file = False
